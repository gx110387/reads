...
@property (nonatomic, weak) GTProgressView *progressView;
@property (nonatomic, weak) GTMenuItem *selItem;
@property (nonatomic, strong) UIColor *bgColor;
@property (nonatomic, strong) NSMutableArray *frames;
@end
// 下划线的高度
static CGFloat const GTProgressHeight = 2.0;
...
- (void)slideMenuAtProgress:(CGFloat)progress {
    ...
    GTMenuItem *currentItem = (GTMenuItem *)[self viewWithTag:tag];
    GTMenuItem *nextItem = (GTMenuItem *)[self viewWithTag:tag+1];
    if (rate == 0.0) {
        rate = 1.0;
        self.selItem.rate = 0;
...
- (void)selectItemAtIndex:(NSInteger)index {
    ...
    GTMenuItem *item = (GTMenuItem *)[self viewWithTag:tag];
    [self.selItem deselectedItemWithoutAnimation];
    self.selItem = item;
    [self.selItem selectedItemWithoutAnimation];
...
- (void)makeStyle {
    switch (self.style) {
        case GTMenuViewStyleLine:
            [self addProgressView];
            break;
        case GTMenuViewStyleFoold:
            [self addFooldViewHollow:NO];
            break;
        case GTMenuViewStyleFooldHollow:
...
- (void)addItems {
    ...
    for (int i = 0; i < self.items.count; i++) {
        ...
        GTMenuItem *item = [[GTMenuItem alloc] initWithFrame:frame];
        item.tag = (i+kTagGap);
        item.delegate = self;
...
- (void)calculateItemFrames {
    CGFloat contentWidth = GTMenuMargin;
    for (int i = 0; i < self.items.count; i++) {
...
- (void)addProgressView {
    GTProgressView *pView = [[GTProgressView alloc] initWithFrame:CGRectMake(0, self.frame.size.height-GTProgressHeight, self.scrollView.contentSize.width, GTProgressHeight)];
...
- (void)addFooldViewHollow:(BOOL)isHollow {
    GTFooldView *fooldView = [[GTFooldView alloc] initWithFrame:CGRectMake(0, 0, self.scrollView.contentSize.width, self.frame.size.height)];
