...
- (void)viewDidLoad {
    ...
    self.rv.regisetButton.enabled = NO;
    [self.rv.regisetButton addTarget:self action:@selector(rightBarAction:) forControlEvents:UIControlEventTouchUpInside];
    [self.rv.isReadSwitch addTarget:self action:@selector(isReadSwitchAction:) forControlEvents:UIControlEventValueChanged];
    [self.rv.mainButton addTarget:self action:@selector(mainButtonAction:) forControlEvents:UIControlEventTouchUpInside];
    [self p_delegate];
    
}
-(void)isReadSwitchAction:(UISwitch *)sender
{
    UISwitch *switchButton = (UISwitch*)sender;
    BOOL isButtonOn = [switchButton isOn];
    if (isButtonOn) {
      
        self.rv.regisetButton.enabled = YES;
    }else {
...
{    AboutUsViewController *gaoxingVC = [[AboutUsViewController alloc] init];
    gaoxingVC.type=@"Y";
    UINavigationController *aboutUsNc = [[UINavigationController alloc] initWithRootViewController:gaoxingVC];
