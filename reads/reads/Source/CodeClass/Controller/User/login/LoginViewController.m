...
- (void)viewDidLoad {
  ...
  self.navigationItem.title =@"用户登陆";
    ...
    [self.lv.myForgetButton addTarget:self action:@selector(myForgetButtonAction:) forControlEvents:UIControlEventTouchUpInside];
...
#pragma mark -忘记密码
-(void)myForgetButtonAction:(UIButton *)sender
{
    ForgetViewController *adviseVC = [[ForgetViewController alloc] init];
                    ...
                    if (user != nil) {
                      //  NSString *name = self.lv.loginTextField.text;
                        ...
                        u.password = self.lv.passTextField.text;
                         ...
                         [tools setUserLogin:u];
                        self.mblock(u);
                        
                      
                        [AlertShow alertShowWithContent:@"登陆成功:" Seconds:1.0f];
